    ...
    populateOrderItems({ user_id: userId, menu_id: menuId, week_of: weekOf, items }) {
      ...
      user.subscriptions
        .forEach((sub) => {
          ...
          const autogeneratedItemChoices = chooseItemsUsingSlots(userSlots, menuItems);

          autogeneratedItemChoices.map(({ slot, item }) => {
            // this is each item/slot pair made for adding to order items
